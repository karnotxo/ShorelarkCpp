/*
    ImFileDialog.cpp
    This file is a modified version of the ImFileDialog.cpp file from the
    ImFileDialog dialog library for Dear ImGui.
...
bool FavoriteButton(const char *label, bool isFavorite) {
    ...
    if (isFavorite || hovered || active) {
        ...
        // since there is no PathFillConcave, fill first the inner part, then
        // the triangles inner
...
FileDialog::FileDialog() {
    ...
    while (userPathStr.back() == L'\0')
        userPathStr.pop_back();
...
bool FileDialog::Save(const std::string &key, const std::string &title, const std::string &filter,
                      const std::filesystem::path &startingDir) {
...
bool FileDialog::Open(const std::string &key, const std::string &title, const std::string &filter,
                      bool isMultiselect, const std::filesystem::path &startingDir) {
    if (!m_currentKey.empty())
        return false;
    ...
    if (hasResult) {
        if (!m_isMultiselect || m_selections.size() <= 1) {
            if (filename.is_absolute())
                m_result.push_back(filename);
            else
                m_result.push_back(m_currentDirectory / filename);
    ...
    for (size_t i = 0; m_previewLoaderRunning && i < m_content.size(); i++) {
        ...
        if (data.Path.has_extension()) {
            ...
            if (ext == ".png" || ext == ".jpg" || ext == ".jpeg" || ext == ".bmp" ||
                ...
                unsigned char *image = stbi_load(data.Path.u8string().c_str(), &width, &height,
    ...
    if (ImGui::BeginPopupModal("Are you sure?##delete")) {
        if (m_selectedFileItem >= static_cast<int>(m_content.size()) || m_content.size() == 0)
            ...
            ImGui::TextWrapped("Are you sure you want to delete %s?",
                               data.Path.filename().u8string().c_str());
            if (ImGui::Button("Yes")) {
                std::error_code ec;
...
const char *ifd::GetDefaultFolderIcon() { return (const char *)&folder_icon[0]; }
